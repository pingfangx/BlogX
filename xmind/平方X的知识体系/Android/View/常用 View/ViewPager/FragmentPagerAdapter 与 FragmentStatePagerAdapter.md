S:
* 分别在 instantiateItem 和 destroyItem 中添加和移除
* 内部是使用 FragmentManager 管理
* FragmentPagerAdapter 初始化时执行的 add 或 attach ，销毁时执行 detach
* FragmentStatePagerAdapter 则分别执行 add 和 remove
* 因为 FragmentStatePagerAdapter 会在销毁时移除，不会持久在 FragmentManager 中，因此适合数量较多的场景
